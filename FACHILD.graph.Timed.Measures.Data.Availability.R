...
table(visits.all$study)

dt.vis <- expand.grid( 
  sjid = unique ( dt.$sjid ),
  avisitn = unique ( dt.$avisitn ),
  paramcd = unique ( dt.$paramcd )
) %>% as_tibble()

dt.vis %<>% 
  left_join(visits.all %>% mutate(occured = 1)) %>% 
  filter(!is.na(occured)) %>% 
  select(study, sjid, avisitn, paramcd)

dt.pct <- dt.vis %>%
  # filter(paramcd == 'w25.i') %>% 
  group_by(study, paramcd, avisitn) %>% 
  mutate( N = n( ) ) %>% # paramcd / visit
  left_join(dt.) %>% 
  group_by(study, avisitn, paramcd, N) %>% 
  filter(!is.na(aval)) %>% 
  summarise( n = n() ) %>% 
...
dt.pct %>% 
  ...
  mutate(param = factor(paramcd, pars., labels = params.)) %>% 
  filter(paramcd %in% pars.[c(6,7,10,11,12)]) %>% 
  ggplot()+geom_point()+geom_line()+
  aes(x = avisitn , y = pct)+scale_x_continuous(breaks = unique(dt.pct$avisitn), labels = levels(dt.pct$avisit))+
  aes(color = param)+
  aes(linetype = study)+
  # facet_wrap(~study)+
  geom_hline(yintercept = c(0,50,100), linetype = 2)+
...
dt.pct %>% 
  # filter(study == 'FACHILD') %>%
  filter( 
    (study == 'FACOMS' & paramcd == 'w25.i') |
      (study == 'FACOMS' & paramcd == 'hpt.i') |
      (study == 'FACHILD' & paramcd %in% c('tug.i','w1m','w6m'))
       ) %>% 
  mutate(param = factor(paramcd, pars., labels = params.)) %>% 
  filter(paramcd %in% pars.[c(6,7,10,11,12)]) %>% 
  ggplot()+geom_col(position = position_dodge(width = 1))+
  aes(x = param , y = pct)+
  aes(fill = param)+
  aes(linetype = study)+
  # facet_wrap(~study)+
...
#   
# dt. %>% 
#   filter(paramcd %in% c(pars.[c(1,6,7,10,11,12,13,14)])) %>% 
#   full_join(dt.vis) %>% 
#   filter(sjid == 4851)
#   
# table(dt.$paramcd)
