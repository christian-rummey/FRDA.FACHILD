...
est.aval %<>% 
  bind_rows(dt.long %>% ungroup %>%  filter(avisit == 'BL') %>% select(intervals, avisit, param) %>% unique() %>%  mutate(estimate = 0)) %>% 
  mutate_at('avisit', factor, visitnames) %>% arrange(param, avisit) %>% 
  mutate( param = factor(param, (params.))) %>%  
  filter( avisit != '3y')

.long.plot.MMRM.int <- function (df, parids, title) {
  ...
  df %>% 
    ...
    aes( color = intervals )+ scale_color_manual(values = c('black','red'))+
    aes( group = paste(param,intervals) )+
...
read_pptx ( '../Templates/CR.template.pptx' ) %>%
  ...
  ph_with   ( dml ( print ( 
    .long.plot.MMRM.int(est.aval, c(1,2,3,4,5),    'Extended Walking, T25FW & Peg Test - MMRM'),
    newpage = F ) ), location = ph_location_type( type = "body" , id = 1) ) %>%
  add_slide ( layout = 'F', master = 'CR')  %>%
  ph_with   ( dml ( print ( 
    .long.plot.MMRM.int(est.aval, c(8,9,10,11,12), 'Rating Scales and ADL - MMRM'),
    newpage = F ) ), location = ph_location_type( type = "body" , id = 1) ) %>%
  print ( target = paste('Changes.MMRM.vs.yearly.', gsub(":","-", Sys.time()), ".pptx", sep="") )
