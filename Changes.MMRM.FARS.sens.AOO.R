...
sjids.31 <- dt.s %>% 
  filter(paramcd == 'FARS.E') %>% filter(avisitn == 0) %>% filter(bl<=31) %>% 
  ungroup %>% select(sjid) %>% deframe

dt.s <- bind_rows(
  dt.s %>% mutate(s.group = 'all'),
  dt.s %>% mutate(s.group = 'FARS.E32') %>% filter(sjid %in% sjids.31)
  ) %>% 
  droplevels()
